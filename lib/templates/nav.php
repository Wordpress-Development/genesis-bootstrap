...
/*
function bsg_genesis_attr_nav_primary( $attr ) 
{
  $attr['class'] .= ( $attr['class'] ? ' ' : '' ) . 'navbar navbar-default navbar-static-top';
  return $attr;
}
add_filter( 'genesis_attr_nav-primary', 'bsg_genesis_attr_nav_primary', 20 );


function bsg_genesis_attr_nav_secondary( $attr ) 
{
  $attr['class'] .= ( $attr['class'] ? ' ' : '' ) . 'navbar navbar-inverse navbar-static-top';
  return $attr;
}
add_filter( 'genesis_attr_nav-secondary', 'bsg_genesis_attr_nav_secondary', 20 );


function bsg_genesis_attr_site_header( $attr ) 
{
  $attr['class'] .= ( $attr['class'] ? ' ' : '' ) . 'container';
  return $attr;
}
add_filter( 'genesis_attr_site-header', 'bsg_genesis_attr_site_header', 20 );
// */
...
function bsg_navbar_brand_logo_customize_register($wp_customize)
{
	...
	$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'bsg_brand_logo', array(
